...
from src.main.ui.dialog.task_dialog import TaskDialog
...
class TasksTab(QWidget):
    ...
    def load_tasks(self):
        ...
        if result['success']:
            ...
            for row, task in enumerate(tasks):
                ...
                self.tasks_table.setItem(row, 2, developer_item)
                self.tasks_table.setItem(row, 3, description_item)
                self.tasks_table.setItem(row, 4, status_item)
                self.tasks_table.setItem(row, 5, hours_item)
    ...
    def apply_filters(self):
        ...
        for row in range(self.tasks_table.rowCount()):
            ...
            project_name = self.tasks_table.item(row, 1).text()
            developer_name = self.tasks_table.item(row, 2).text()
            task_status = self.tasks_table.item(row, 4).text()
            
            # Проверка соответствия фильтрам
            description_match = search_text in description
            project_match = not project_id or str(project_id) == self.tasks_table.item(row, 0).text()
            developer_match = not developer_id or developer_name != "Не назначен"
            status_match = not status or task_status == status
            
            # Отображение/скрытие строки
            self.tasks_table.setRowHidden(row, not (description_match and project_match and developer_match and status_match))
